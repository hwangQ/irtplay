...
rdif2 <- function(x, data, score=NULL, se=NULL, reg.cr=FALSE, group, focal.name, D=1, alpha=0.05, missing=NA, purify=FALSE, 
  ...
  dif_rst <- rdif_one2(x=x, data=data, score=score, se=se, reg.cr=reg.cr, 
                       group=group, focal.name=focal.name, D=D, alpha=alpha)
...
rdif_one2 <- function(x, data, score, se, reg.cr, group, focal.name, D=1, alpha=0.05) {
  ...
  if(reg.cr) {
    ...
    score_ref <- mu_ref + rho_ref^2 * (score_ref - mu_ref)
    score_foc <- mu_foc + rho_foc^2 * (score_foc - mu_foc)
    # score_ref[is.nan(score_ref)] <- NA_real_
    # score_foc[is.nan(score_foc)] <- NA_real_
  ...
  # insert NA values into the missing data location
  ...
  # check sample size
  n_ref <- colSums(!is.nan(resid_ref)|!is.na(resid_ref))
  n_foc <- colSums(!is.nan(resid_foc)|!is.na(resid_foc))
  ...
  rdif_r <- colMeans(resid_foc, na.rm=TRUE) - colMeans(resid_ref, na.rm=TRUE) # the difference of mean raw residuals
  rdif_s <- colMeans(resid_foc^2, na.rm=TRUE) - colMeans(resid_ref^2, na.rm=TRUE) # the difference of mean squared residuals
