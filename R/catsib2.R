...
catsib2 <- function(x=NULL, data, score=NULL, se=NULL, level.rel="group", group, focal.name, D=1, n.bin=c(80, 10), min.binsize=3, max.del=0.075, 
  ...
  dif_rst <- 
    catsib_one2(data=data, group=group, focal.name=focal.name, score=score, se=se, 
                level.rel=level.rel, range=range, item.id=item.id, max.bin=max.bin, 
                min.bin=min.bin, min.binsize=min.binsize, 
  ...
  if(purify) {
    ...
    if(!is.null(dif_item_tmp)) {
      ...
      for(i in 1:max.iter) {
        ...
        item.id <- x_puri$id
        dif_rst_tmp <- 
          catsib_one2(data=data_puri, group=group, focal.name=focal.name, score=score_puri, se=se_puri, 
                      level.rel=level.rel, range=range, item.id=item.id, max.bin=max.bin, min.bin=min.bin, 
                      min.binsize=min.binsize, max.del=max.del, weight.group=weight.group, alpha=alpha)
...
catsib_one2 <- function(data, group, focal.name, score, se, level.rel="group", range,
